...
##1.1 Browse time series to establish events manually
```{r}
...
start <- int_start(r2_events2$start_time_EST[1])
class(start)
tz(start)
...
#Assign event numbers/filter each hobo record based on event intervals ----
...
for (i in 1:length(intervals$hobo_event_n)) {
  interval <- hobo_rec %>%
    filter(dt2 %within% intervals$datetime_interval_EST[i]) %>% 
