...
hobo_smooth <- hobo_norm_wide %>%
  mutate(SFA= smoothing(SFA_mm, method = "loess", strength = 0.01))%>%
  mutate(SFB= smoothing(SFB_mm, method = "loess", strength = 0.01))%>%
  mutate(SFC= smoothing(SFC_mm, method = "loess", strength = 0.01))%>%
  mutate(SFD= smoothing(SFD_mm, method = "loess", strength = 0.01))%>%
  mutate(TFB= smoothing(TFB_mm, method = "loess", strength = 0.01))%>%
  mutate(TFD= smoothing(TFD_mm, method = "loess", strength = 0.01))%>%
  select(SFA,SFB,SFC,SFD,TFB,TFD,
         hobo_event_n,dt)%>%
  pivot_longer(cols = contains("F"), names_to = "site", 
               values_to = "yield_mm") %>%
  group_by(site)%>%
  ungroup()

saveRDS(hobo_smooth, paste0(here, "/output/hobo_events.Rds"))
```
